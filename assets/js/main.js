...
for (let i = 0; i < NBR_PARTICLE_AMOUNT; i++) {
	...
	elemParticle.style.borderRadius = `${BOOL_PARTICLE_IS_ROUNDED ? (Math.max(NBR_PARTICLE_WIDTH_MAX, NBR_PARTICLE_HEIGHT_MAX) + NBR_PARTICLE_DEPTH_OFFSET) / 2 : Math.round(NBR_PARTICLE_BORDER_RADIUS_MIN + (NBR_PARTICLE_BORDER_RADIUS_MAX - NBR_PARTICLE_BORDER_RADIUS_MIN) * i/NBR_PARTICLE_AMOUNT)}px`;
