...
for (let i = 0; i < NBR_PARTICLE_AMOUNT; i++) {
	...
	elemParticle.style.width = `${
		Math.round(
			randomizeMinMaxStep(NBR_PARTICLE_WIDTH_MIN, NBR_PARTICLE_WIDTH_MAX, NBR_PARTICLE_WIDTH_STEP) + 
			i/NBR_PARTICLE_AMOUNT * NBR_PARTICLE_DEPTH_OFFSET * 
			(NBR_PARTICLE_WIDTH_MAX - NBR_PARTICLE_WIDTH_MIN) / 
			(NBR_PARTICLE_WIDTH_MAX - NBR_PARTICLE_WIDTH_MIN + NBR_PARTICLE_HEIGHT_MAX - NBR_PARTICLE_HEIGHT_MIN)
		)
	}px`;
	elemParticle.style.height = BOOL_PARTICLE_HAS_SIDES_EQUAL? elemParticle.style.width : `${
		Math.round(
			randomizeMinMaxStep(NBR_PARTICLE_HEIGHT_MIN, NBR_PARTICLE_HEIGHT_MAX, NBR_PARTICLE_HEIGHT_STEP) + 
			i/NBR_PARTICLE_AMOUNT * NBR_PARTICLE_DEPTH_OFFSET * 
	...
	elemParticle.style.borderRadius = `${BOOL_PARTICLE_IS_ROUNDED ? 
			(Math.max(NBR_PARTICLE_WIDTH_MAX, NBR_PARTICLE_HEIGHT_MAX) + NBR_PARTICLE_DEPTH_OFFSET) / 2 : 
			Math.round(
	...
	elemParticle.style.opacity = `${BOOL_PARTICLE_IS_OPAQUE ? 
			...
			Math.trunc(
				i/NBR_PARTICLE_AMOUNT * 
				(NBR_PARTICLE_OPACITY_MIN + (NBR_PARTICLE_OPACITY_MAX - NBR_PARTICLE_OPACITY_MIN)) * 
				100
			) / 100
	}`;
