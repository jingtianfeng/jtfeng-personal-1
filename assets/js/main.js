...
for (let i = 0; i < NBR_PARTICLE_AMOUNT; i++) {
	...
	elemParticle.style.width = `${Math.round(randomizeMinMaxStep(NBR_PARTICLE_WIDTH_MIN, NBR_PARTICLE_WIDTH_MAX, NBR_PARTICLE_WIDTH_STEP) + i/NBR_PARTICLE_AMOUNT*NBR_PARTICLE_DEPTH_OFFSET)}px`;
	elemParticle.style.height = BOOL_PARTICLE_HAS_SIDES_EQUAL? elemParticle.style.width : `${Math.round(randomizeMinMaxStep(NBR_PARTICLE_HEIGHT_MIN, NBR_PARTICLE_HEIGHT_MAX, NBR_PARTICLE_HEIGHT_STEP) + i/NBR_PARTICLE_AMOUNT*NBR_PARTICLE_DEPTH_OFFSET)}px`;
